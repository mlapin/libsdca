#pragma once
/**
 @ brief f*ast math library for float
 @author herumi
...
namespace fmath {
  ...
  namespace local {
    ...
    /*
     e xp(88.7*22839f) = inf ; 0x42b17218
    ...
    struct ExpVar {
      ...
      ExpVar()
      {
        ...
        for (int i = 0; i < n; i++) {
          float y = (float)pow(2.0f, (float)i / n);
          fi fi;
          fi.f = y;
          tbl[i] = fi.i & mask(23);
        }
    ...
    struct ExpCode : public Xbyak::CodeGenerator {
      ...
      void makeExpPs(const ExpVar<N> *self, const Xbyak::util::Cpu& cpu)
      {
        ...
        /*
         i f abs(x*) >= maxX then x = max(min(x, maxX), -maxX) and try
  ...
  inline double expd(double x)
  {
    ...
    #else
    /*
     r emark :* -ffast-math option of gcc may generate bad code for fmath::expd
     */
