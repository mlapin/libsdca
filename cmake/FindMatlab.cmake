...
function(matlab_add_mex_use_compiler)
  ...
  set(oneValueArgs NAME DOCUMENTATION OUTPUT_NAME)
  set(multiValueArgs LINK_TO SRC DEPENDS)
  ...
  set(${prefix}_FULL_NAME "${${prefix}_OUTPUT_NAME}.${Matlab_MEX_EXTENSION}")

  set(${prefix}_SRC_ABS)
  foreach(__t ${${prefix}_SRC})
    get_filename_component(__tt ${__t} ABSOLUTE)
    list(APPEND ${prefix}_SRC_ABS ${__tt})
  endforeach()

  set(${prefix}_INC_DIR)
  get_property(__tt DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR} PROPERTY INCLUDE_DIRECTORIES)
  foreach(__t ${__tt})
    list(APPEND ${prefix}_INC_DIR "-I${__t}")
  endforeach()

  string(TOUPPER ${CMAKE_BUILD_TYPE} __config)
  set(__t "${CMAKE_CXX_FLAGS} ${CMAKE_CXX_FLAGS_${__config}}")
  set(__tt "${__t} ${CMAKE_SHARED_LINKER_FLAGS}")

  set(${prefix}_FLAGS)
  list(APPEND ${prefix}_FLAGS CXX="${CMAKE_CXX_COMPILER}")
  list(APPEND ${prefix}_FLAGS CXXFLAGS="-fPIC ${__t}")
  list(APPEND ${prefix}_FLAGS LDFLAGS="-fPIC ${__tt}")
  list(APPEND ${prefix}_FLAGS -largeArrayDims)

  get_property(__tt DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR} PROPERTY COMPILE_DEFINITIONS)
  foreach(__t ${__tt})
    list(APPEND ${prefix}_FLAGS -D'${__t}')
  endforeach()

  add_custom_command(
    OUTPUT ${${prefix}_FULL_NAME}
    COMMAND ${Matlab_MEX_COMPILER} ARGS
      -output ${${prefix}_FULL_NAME}
      ${${prefix}_FLAGS}
      ${${prefix}_INC_DIR}
      ${${prefix}_SRC_ABS}
      ${${prefix}_LINK_TO}
    DEPENDS ${${prefix}_SRC} ${${prefix}_DEPENDS}
    COMMENT "Compiling: ${${prefix}_FULL_NAME}"
    VERBATIM
    )

  add_custom_target(${${prefix}_NAME} ALL
    DEPENDS ${${prefix}_FULL_NAME}
    SOURCES ${${prefix}_SRC} ${${prefix}_DEPENDS}
    )
...
# Component BLAS library
list(FIND Matlab_FIND_COMPONENTS BLAS_LIBRARY _matlab_find_blas)
if(_matlab_find_blas GREATER -1)
  _Matlab_find_library(
    ${_matlab_lib_prefix_for_search}
    Matlab_BLAS_LIBRARY
    mwblas
  ...
  if(Matlab_BLAS_LIBRARY)
    set(Matlab_BLAS_LIBRARY_FOUND TRUE)
  endif()
endif()
